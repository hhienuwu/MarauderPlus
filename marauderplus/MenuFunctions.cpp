...
/* Interrupt driven periodic handler */
  ...
  void MenuFunctions::addAPGFX(String type){
    ...
    // Add "Select All" button with toggle state
    static bool select_all_state = false;
    lv_obj_t * select_all_btn = lv_list_add_btn(list1, LV_SYMBOL_OK, select_all_state ? "Deselect All" : "Select All");
    lv_btn_set_checkable(select_all_btn, true);
    
    lv_obj_set_event_cb(select_all_btn, [](lv_obj_t * btn, lv_event_t event) {
      static bool select_all_state = false;
      if (event == LV_EVENT_CLICKED) {
        extern LinkedList<AccessPoint>* access_points;
        ...
        lv_label_set_text(label, select_all_state ? "Deselect All" : "Select All");

        // Get the parent list object
        lv_obj_t * list = lv_obj_get_parent(btn);
        ...
        while(next_btn != NULL) {
          // Get button text
          const char * btn_text = lv_list_get_btn_text(next_btn);

          // Skip if it's the Back button or Select/Deselect All button
          // Handle state for regular AP buttons
          if (strcmp(btn_text, text09) != 0 && 
              strcmp(btn_text, "Select All") != 0 && 
              strcmp(btn_text, "Deselect All") != 0) {
            
            // Set button state based on select_all_state
            if(select_all_state) {
              lv_btn_set_state(next_btn, LV_BTN_STATE_CHECKED_RELEASED);
            } else {
              lv_btn_set_state(next_btn, LV_BTN_STATE_RELEASED);
            }
            ...
            for (int i = 0; i < access_points->size(); i++) {
              ...
              if (ap.essid == String(btn_text)) {
                ap.selected = select_all_state;
                access_points->set(i, ap);
                break;
              }
            }
          }
          // Reset state for Select/Deselect All button
          else if (strcmp(btn_text, "Select All") == 0 || strcmp(btn_text, "Deselect All") == 0) {
    ...
    // Hide "Select All" button if the list is empty 
    if (access_points->size() == 0) {
      lv_obj_set_hidden(select_all_btn, true);
    }
  }
