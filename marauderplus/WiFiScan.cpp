...
extern "C" {
  ...
  NimBLEAdvertisementData WiFiScan::GetUniversalAdvertisementData(EBLEPayloadType Type) {
    ...
    switch (Type) {
      //Custom SwiftPair spam list by DanhHien
      case Microsoft: {
          // List of device names for spamming
          const char* names[] = {
              "uy iPhone",
              "Cá lóc 🐟",
              "iPhone anh Phúc",
...
void WiFiScan::RunBeaconScan(uint8_t scan_mode, uint16_t color)
{
  if (scan_mode == WIFI_SCAN_AP)
    startPcap("beacon");
      ...
      if (gps_obj.getGpsModuleStatus()) {
        startLog("wardrive");
        String header_line = "WigleWifi-1.4,appRelease=" + (String)MARAUDER_VERSION + ",model=ESP32 CartonBoard,release=" + (String)MARAUDER_VERSION + ",device=ESP32 CartonBoard,display=SPI TFT,board=ESP32 DanhHien,brand=DanhHien\nMAC,SSID,AuthMode,FirstSeen,Channel,RSSI,CurrentLatitude,CurrentLongitude,AltitudeMeters,AccuracyMeters,Type\n";
  ...
  if (scan_mode == WIFI_SCAN_PROBE)
    startPcap("probe");
      ...
      if (gps_obj.getGpsModuleStatus()) {
        ...
        String header_line = "WigleWifi-1.4,appRelease=" + (String)MARAUDER_VERSION + ",model=ESP32 DanhHien,release=" + (String)MARAUDER_VERSION + ",device=ESP32 CartonBoard,display=SPI TFT,board=ESP32 CartonBoard,brand=DanhHien\nMAC,SSID,AuthMode,FirstSeen,Channel,RSSI,CurrentLatitude,CurrentLongitude,AltitudeMeters,AccuracyMeters,Type\n";
        buffer_obj.append(header_line);
      } else {
        return;
      }
